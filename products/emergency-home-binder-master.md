# LivSolv Emergency Home Binder
## Live in the solution.

PRODUCT STATUS: Working master, September 2026

PURPOSE
This is not a generic preparedness checklist. It is a household operating manual for emergencies: what to do first, where critical things are, what the household has, what it lacks, and how to keep functioning when phones, internet, power, water or normal services are unavailable.

DESIGN RULES
- Emergency-first: the most time-critical pages must be usable in seconds.
- Paper-first: critical instructions cannot depend on a charged phone or internet.
- Household-specific: blanks and worksheets turn generic advice into an actual plan.
- Capability-first: HAVE IT / KNOW WHERE IT IS / KNOW HOW TO USE IT / NEED IT / $0 ALTERNATIVE / PRIORITY.
- Low cognitive load: large headings, checkboxes, short instructions, tabbed sections.
- Separate sensitive documents from the everyday reference binder where appropriate. The binder should tell the household where protected originals/copies are stored without unnecessarily exposing identity or financial data.
- Include a secure digital-backup plan for important records, but never make digital access the only copy.
- Review dates on pages that expire or change.
- Current product recommendations live on LivSolv.com so the printed binder does not become obsolete.

FRONT OF BINDER: USE THIS FIRST
1. Emergency Dashboard
2. Five-Minute Evacuation Card
3. Fifteen-Minute Evacuation Card
4. Sixty-Minute Evacuation Card
5. Family Contact + Rally Card
6. Utility Shutoff Quick Card
7. Medical Emergency Information Card
8. Local Map + primary/alternate evacuation routes

SECTION 1: HOUSEHOLD SNAPSHOT
- Household members and contact information
- Important medical/allergy information
- Pets and animal needs
- Emergency contacts
- Out-of-area contact
- Neighbors/support network
- School/work locations and reunification information
- Household roles during an emergency

SECTION 2: EVACUATION
- Trigger: what conditions mean we leave?
- 5-minute / 15-minute / 60-minute departure checklists
- Loading-the-car order
- Go-bag locations and inventory
- Medication grab list
- Pet evacuation plan
- Documents grab location
- Primary route + at least two alternates
- Paper maps
- Rally points: neighborhood / local / out-of-area
- Destination options in multiple directions
- Fuel/refueling plan
- Arrival checklist
- Return-home checklist

SECTION 3: SHELTER-IN-PLACE / HOME EMERGENCY
- First 10 minutes / first hour / first night / 24 hours / 72 hours / one week+
- When staying becomes more dangerous than leaving

SECTION 4: WATER
- People + pets water calculation
- Stored water inventory and locations
- Portable vs bulk storage
- Rapid-fill plan before a storm
- Nearby possible water sources
- IMPORTANT: natural water and improvised seep wells remain untreated water
- Treatment methods available in this household
- Boiling capability + fuel
- Filter/purifier model, replacement schedule and freeze/storage notes
- Clean/dirty container separation
- Water ration/usage worksheet without unsafe dehydration targets
- Current recommendations: LivSolv.com/emergency-water-treatment.html

SECTION 5: FOOD
- No-cook / quick-cook / long-term food inventory
- Household dietary restrictions
- First foods to eat when power fails
- Fridge/freezer preservation plan
- Cooler/ice plan
- Manual can opener location
- Emergency meal matrix
- Fuel/water burden notes
- Pantry rotation dates

SECTION 6: BLACKOUT COOKING
- Primary + backup cooking method
- Fuel type and stored quantity
- Safe operating location
- Ignition backup
- Cookware that fits backup stove
- Retained-heat cooking instructions
- Solar cooking option
- No-cook fallback
- Carbon-monoxide warning
- Current guide: LivSolv.com/cooking-during-power-outage.html

SECTION 7: POWER + GENERATOR
Critical loads
- Rank household loads 1-5
- Note running watts and startup/surge needs where relevant
- Mark equipment requiring 240V or special connection
- Refrigerator/freezer cycling plan
- Battery/power-station charging priority

Generator identity
- Make/model/serial
- Gasoline running/starting watts
- Propane running/starting watts if dual-fuel
- Outlet types, including RV-ready outlet if present
- Generator location in storage
- Safe outdoor operating location
- Who in household can physically move/set it up
- Wheel/handle status

Fuel redundancy plan
- Fuel types generator can use
- Gasoline currently stored + date acquired/rotated
- Propane cylinders currently available + size
- Do NOT assume propane is always cheaper; compare current local fuel cost against actual generator consumption
- Do NOT assume gasoline will be available during a storm/evacuation; fill/prepare before emergency demand spikes when safe and practical
- Dual-fuel advantage: two supply options, not a promise of lower operating cost
- Note lower propane wattage if applicable
- Estimated generator-hours available on each stored fuel

Runtime strategy
- How many generator-hours per day are actually required?
- Which high-load appliances run only during generator periods?
- Which batteries/power stations get recharged while generator is running?
- Which loads can run from battery while generator is off?
- Fridge/freezer thermometer plan so cycling is based on temperature rather than guessing
- Quiet/night shutdown plan where practical

Startup card: IF THE PERSON WHO NORMALLY DOES THIS IS NOT HERE
- Move generator to designated safe outdoor location
- Verify oil/fuel according to model manual
- Connect only through the household's documented safe connection method
- Exact model-specific fuel/start/choke/electric-start sequence
- What loads get connected first
- What NOT to connect
- Where spare oil, cords and adapters are stored
- Shutdown/cool-down/refueling sequence from manufacturer instructions
- Emergency stop instruction
- Never improvise backfeeding

Safety
- Generator never operates inside house, garage, basement, crawlspace or other enclosed/partially enclosed area
- Keep away from doors/windows/vents per manufacturer guidance
- CO alarms: locations + last test
- Rain/wet-weather protection method that does not enclose exhaust
- Fuel stored according to applicable container/fire-safety requirements
- Transfer switch/interlock/connection equipment and electrician information where applicable

Maintenance
- Owner's manual location
- Oil type
- Spare oil quantity/location
- Last oil change
- Last test run
- Battery condition if electric start
- Spark plug/air filter service notes
- Fuel rotation date
- Next scheduled practice setup
- Current guide: LivSolv.com/generator-backup-power.html

SECTION 8: EXTREME COLD / HEAT
- Winter warm-room selection and isolation
- Indoor tent/sleeping microclimate plan
- Blankets/sleeping bags/pads location
- Safe heat sources
- CO/smoke alarm locations
- Summer cool-zone selection
- Sun-blocking plan
- Night-flush ventilation plan when conditions/air quality permit
- Fans/cooling gear
- Triggers for relocating to safer conditioned space

SECTION 9: COMMUNICATIONS
- Primary / Alternate / Contingency / Emergency communication methods
- Family contact tree / out-of-area contact / meeting places
- NOAA weather radio information
- FRS/GMRS/ham/satellite equipment actually owned
- Channels/frequencies/call signs where legally applicable
- Charging/battery plan
- Prewritten short emergency messages
- Paper phone-number list

SECTION 10: MEDICAL / FIRST AID
- Main kit + refill locations
- Medications/allergies/conditions summary
- Household-specific medical equipment
- Prescription refill/backup plan from clinician/pharmacist
- Refrigerated medication power/cold plan where applicable
- First-aid/CPR trained household members
- Printed recognized first-aid quick reference
- Kit expiration/restock review date
- Emergency communication alternatives
- Current kit recommendations: LivSolv.com/best-home-first-aid-kits.html

SECTION 11: SANITATION / HYGIENE
- Toilet plan if water/sewer unavailable
- Handwashing/hygiene plan
- Trash/waste containment
- Menstrual products
- Infant/elder-care supplies if applicable
- Cleaning/disinfection supplies
- Gloves
- Laundry strategy for extended outage

SECTION 12: VEHICLES
- Vehicle identification
- Fuel rule/target
- Jump starter/cables location
- Inflator/tire tools
- Spare tire/jack instructions
- Seasonal car kit
- Water/food/blankets
- Paper maps
- Vehicle maintenance dates
- Roadside assistance/insurance contacts

SECTION 13: HOME SYSTEMS
- Main water shutoff: location + photo/diagram
- Electrical main: location + instructions
- Gas shutoff: location + utility-specific guidance
- Well/septic information
- Generator transfer equipment
- HVAC basics
- Fire extinguishers
- Smoke/CO alarm maintenance
- Sump pump
- Important appliance model numbers
- Trusted repair/service contacts

SECTION 14: DOCUMENTS / RECOVERY
Keep highly sensitive originals and full financial details protected. This section can contain a grab-location index plus carefully selected copies.
- IDs/passports/birth/marriage records
- Insurance policies/cards and claim contacts
- Property/deed/lease/mortgage information
- Vehicle titles/registrations
- Medical insurance information
- Pet records
- Wills/directives/POA location
- Recent home inventory/photos/video location
- Proof-of-residence documents
- Secure digital backup location/instructions
- Recovery receipts envelope
- Disaster claim log

SECTION 15: MONEY / FINANCIAL CONTINUITY
- Emergency cash location (reference may be coded/general rather than exact if desired)
- Essential recurring bills
- Insurance deductibles
- Emergency savings target
- Financial institution contact list without unnecessary passwords
- Temporary lodging/payment plan
- Expense/receipt log after disaster

SECTION 16: PETS / ANIMALS
- Carriers/leashes location
- Food/water calculation
- Medications
- Vet/emergency vet
- Vaccination/medical records
- Identification/microchip information
- Evacuation destinations that accept animals
- Comfort/containment supplies
- Animal-specific transport instructions

SECTION 17: SKILLS / QUICK REFERENCES
Only concise, reputable instructions appropriate for lay use.
- First-aid reference
- Water treatment decision tree
- Fire extinguisher basics
- Utility shutoff
- Generator startup/shutdown for owned equipment
- Jump-start procedure
- Tire change procedure
- Emergency radio basics
- Equipment-specific instructions

SECTION 18: INVENTORY GAPS + MAINTENANCE
- What capability is missing?
- $0 workaround / cheapest workable fix / better-value purchase
- Priority: now / soon / later
- Last checked date
- Water/food/medication rotation
- Battery charging
- Generator test + fuel rotation
- Smoke/CO alarms
- Vehicle kit seasonal change
- Insurance/home-inventory review
- Family drill/review

COMMERCIAL / WEBSITE INTEGRATION
Printed material should avoid hard-coding product models that may become obsolete. Use stable LivSolv landing pages and QR codes for current comparisons. Those web pages can contain properly disclosed affiliate links. The binder remains useful even if a reader never buys anything.

PRODUCT VERSIONS TO DEVELOP
A. Printable PDF: affordable instant-download edition.
B. Premium printed binder/workbook: tabbed, durable, writable, designed to live in the home.
C. Book companion pack: selected worksheets bundled with the LivSolv preparedness book.
D. Free sampler: Emergency Dashboard + 5/15/60-minute evacuation cards + household contact card. Use as an email-list acquisition product only after email infrastructure is justified.

RESEARCH NOTES INCORPORATED
- Experienced preparedness communities repeatedly emphasize paper maps, multiple evacuation routes, household roles, offline instructions, equipment procedures and keeping the binder maintained rather than treating it as a one-time checklist.
- Generator users repeatedly surface fuel storage/availability, load management, cord capacity, physical mobility and household familiarity as practical failure points.
- Dual-fuel is treated as supply redundancy, not automatically cheaper fuel. Propane output is often lower, so model-specific ratings matter.
- A practical security distinction is useful: everyday emergency operating instructions can live in an accessible binder while sensitive originals/full financial records stay in a protected grab file or secure storage.

NEXT BUILD
Continue designed worksheet copy, including a two-page Generator & Fuel Plan with model-specific startup card and a simple generator-hours-per-day worksheet.